...
export function mountSharedShipmentResult(container, input) {
  ...
  container.innerHTML = `<section class="shared-shipment-result" aria-labelledby="shipment-result-title"><div class="success-mark" aria-hidden="true">✓</div><p class="eyebrow">Shipment purchased</p><h2 id="shipment-result-title">Your shipment is on its way</h2><div class="tracking-card"><div><small>REFERENCE</small><strong>${escapeHtml(model.shipment.reference)}</strong></div><div><small>TRACKING NUMBER</small><strong>${escapeHtml(model.shipment.trackingNumber)}</strong></div>${tracking ? `<a class="primary button-link" href="${escapeHtml(tracking)}" target="_blank" rel="noopener noreferrer">Track shipment</a>` : ''}</div><div class="panel payment-record" role="status"><div class="payment-record-head"><div><small>PAYDEMO FULL ORDER + DELIVERY</small><h3>Payment recorded</h3></div><span class="payment-status-badge">Paid</span></div><strong class="payment-record-total">${escapeHtml(money(model.payment.total, model.payment.currency))}</strong><dl><div><dt>Reference</dt><dd>${escapeHtml(model.payment.reference)}</dd></div><div><dt>Merchandise</dt><dd>${escapeHtml(money(model.payment.merchandise, model.payment.currency))}</dd></div><div><dt>Delivery</dt><dd>${escapeHtml(money(model.payment.delivery, model.payment.currency))}</dd></div><div><dt>Insurance</dt><dd>${escapeHtml(insurance)}</dd></div><div><dt>Carrier</dt><dd>${escapeHtml(model.payment.carrier)}</dd></div><div><dt>Confirmed</dt><dd>${escapeHtml(model.payment.confirmedAt || 'Confirmed by PayDemo')}</dd></div></dl></div><h3>Shipment documents</h3><div class="documents">${model.documents.map((document) => documentCard(document, urls)).join('')}</div></section>`;
