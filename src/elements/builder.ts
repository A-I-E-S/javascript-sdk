...
export class AfricaniesShipmentBuilderElement extends AfricaniesElement {
  ...
  protected render(): void {
    ...
    this.bind();
    if (this.#editor) queueMicrotask(()=>this.root.querySelector<HTMLInputElement>('[data-editor-initial="true"], dialog input, dialog button')?.focus());
  }
  ...
  private renderEditor(): string {
    ...
    if(editor.kind==='box'){
      ...
      return `<dialog open aria-labelledby="editor-title" aria-describedby="editor-description"><section class="card stack"><div class="section-title"><div><h3 id="editor-title">${editor.added?'Add':'Edit'} box</h3><p id="editor-description" class="muted">Enter package dimensions and gross weight.</p></div><button type="button" class="secondary" data-action="cancel-editor" aria-label="Close editor">×</button></div>${editor.error?`<p class="alert error" role="alert">${escapeHtml(editor.error)}</p>`:''}<div class="grid">${(['length','width','height','weight'] as const).map(key=>`<label>${key==='weight'?'Gross weight':key[0]!.toUpperCase()+key.slice(1)}<input data-editor-initial="${key==='length'}" inputmode="decimal" data-box="${editor.box}" data-box-field="${key}" value="${escapeHtml(box[key])}"></label>`).join('')}</div><div class="actions"><button type="button" class="secondary" data-action="cancel-editor">Cancel</button><button type="button" class="primary" data-action="save-editor">Save box</button></div></section></dialog>`;
    }
    const itemIndex=editor.item!;const item=this.#value!.boxes[editor.box]!.items[itemIndex]!;
    return `<dialog open aria-labelledby="editor-title" aria-describedby="editor-description"><section class="card stack"><div class="section-title"><div><h3 id="editor-title">${editor.added?'Add':'Edit'} item</h3><p id="editor-description" class="muted">Use an everyday name and select the closest Products API classification.</p></div><button type="button" class="secondary" data-action="cancel-editor" aria-label="Close editor">×</button></div>${editor.error?`<p class="alert error" role="alert">${escapeHtml(editor.error)}</p>`:''}${this.renderItem(item,editor.box,itemIndex)}<div class="actions"><button type="button" class="secondary" data-action="cancel-editor">Cancel</button><button type="button" class="primary" data-action="save-editor">Save item</button></div></section></dialog>`;
  }
