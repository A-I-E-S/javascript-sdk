...
export class AfricaniesRateSelectionElement extends AfricaniesElement {
  ...
  protected render(): void {
    ...
    this.root.innerHTML = `<style>${sharedStyles}
      ...
      <div class="stack">${state?.rates.map((rate) => `<article class="card rate ${state.selectedSlug === rate.slug ? 'selected' : ''}"><div><h3>${escapeHtml(rate.name)}</h3><p class="muted">${escapeHtml(rate.others.min_day)}–${escapeHtml(rate.others.max_day)} business days · ${escapeHtml(rate.slug)}</p></div><div><div class="price">${escapeHtml(rate.others.currency)} ${escapeHtml(rate.payment_amount.toFixed(2))}</div><button class="${state.selectedSlug === rate.slug ? 'secondary' : 'primary'}" data-slug="${escapeHtml(rate.slug)}" type="button" aria-pressed="${state.selectedSlug === rate.slug}" ${state.status === 'loading' ? 'disabled' : ''}>${state.selectedSlug === rate.slug ? 'Selected' : 'Select rate'}</button></div></article>`).join('') ?? ''}</div>
  ...
  private connectController(autoLoad = true): void {
    ...
    if (this.isConnected) {
      ...
      if (autoLoad) void this.#controller.load().catch(() => undefined);
    }
  }
}
